...
public class BiObservable<T0, T1> {
    ...
    /**
     * Composes an Rx operator's effect to a to a subscriber and returns a new subscriber. This
     * operator applies its effect to a two valued {@link DualSubscriber subscriber} and returns a
     * new subscriber of the same kind.
     * 
     * @param <R0>
     ...
     * transforms a two valued {@link DualSubscriber subscriber} and adapts it to a single valued
     ...
     * {@link BiSubscriber} or {@link DualSubscriber}.
     *
     * @param <T0>
     *            type of first argument
     * @param <T1>
     ...
     * {@link DualSubscriber subscriber} that applies the given operator's effect to values produced
     * when subscribed to.
     * 
     * @param dualOperator
    ...
    public static <T0, T1> BiObservable<T0, T1> generate(final Observable<? extends T0> observable, final Func1<? super T0, ? extends T1> generatorFunc) {
        return BiObservable
                .lift(observable, new OperatorGenerate<T0, T1>(generatorFunc));
    }
